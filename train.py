...
def parse_args():
    ...
    parser.add_argument("--shaping", type=str, default="234", help="reward shaping method, 0 for no shaping; 1 for add future reward,i.e. R= r+R*gamma; 2 for use finalreward; 3 for subtract baseline(value of curent state) 1234 means all the method is used,")
...
class SingleTrain(object):
    def __init__(self, args):
        ...
        for i, dataset in enumerate(self.datasets):
            for index in args.multigraphindex:
                g = GraphLoader(dataset, sparse=True, args=args, multigraphindex=f'graph{index}')
                g.process()
